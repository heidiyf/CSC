...
from a2_wordle_helpers import INCORRECT, CORRECT, WRONG_POSITION, cartesian_product_general
...
def is_wrong_position_char(answer: str, guess: str, i: int) -> bool:
    ...
    True
    """
    return guess[i] != answer[i] and guess[i] in answer
...
def is_incorrect_char(answer: str, guess: str, i: int) -> bool:
    ...
    """
    return not is_correct_char(answer, guess, i)
...
def is_correct_single(word: str, guess: str, status: list[str]) -> bool:
    ...
    False
    """
    statuses = get_guess_status(word, guess)
...
def is_correct_multiple(word: str, guesses: list[str], statuses: list[list[str]]) -> bool:
    ...
    True
    """
    return statuses == get_guesses_statuses(word, guesses)

    # for guess in guesses:
    #     if is_correct_single(word, guess, get_guess_status(word, guess)):
    #         return True
    # return False
...
def find_correct_answers(word_set: set[str],
    ...
    ['later', 'liter']
    """
    return sorted([word for word in word_set if is_correct_multiple(word, guesses, statuses)])

    # answer_so_far = []
    # for word in word_set:
    #     if is_correct_multiple(word, guesses, statuses):
    #         answer_so_far = answer_so_far + [word]
...
def part3b_example(word_set_file: str, guesses: list[str], statuses: list[list[str]]) -> None:
    ...
    answer = find_correct_answers(word_set, guesses, statuses)
    a2_visualizer.draw_wordle_answers(answer, guesses, statuses)
...
def find_correct_guesses_single(word_set: set[str], answer: str, status: list[str]) -> list[str]:
    ...
    ['tiger', 'tower']
    """
    return sorted([guess for guess in word_set if is_correct_single(answer, guess, status)])
...
def find_correct_guesses_multiple(word_set: set[str],
    ...
    """
    possible_guesses = [find_correct_guesses_single(word_set, answer, status) for status in statuses]
    return cartesian_product_general(possible_guesses)
...
def part3c_example(word_set_file: str, answer: str, statuses: list[list[str]]) -> None:
    ...
    all_guess = find_correct_guesses_multiple(word_set, answer, statuses)
    a2_visualizer.draw_wordle_guesses(answer, all_guess, statuses)
...
def find_correct_answers_and_scores(word_set: set[str],
    ...
    return {possible_answer: average_score(possible_answer, possible_answers) for possible_answer in possible_answers}


@check_contracts
def average_score(answer: str, guesses: list[str]) -> float:
    """ return the average score for each guess word compare to the answer.

    preconditions:
    - guesses != []

    """
    return sum([information_score(answer, guess) for guess in guesses]) / len(guesses)
