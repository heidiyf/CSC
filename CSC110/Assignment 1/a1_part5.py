...
def crop_image(image_data: list, start_row: int, start_col: int, crop_height: int, crop_width: int) -> list:
    ...
    """
    return [[a1_part4.crop_row(image_data[x], start_col, crop_width)]
            for x in range(start_row, crop_height + start_row)]

    # return [[image_data[i][x] for i in range(start_row, start_row + crop_height)]
    #         for x in range(start_col, start_col + crop_width)]
