...
def crop_row_border_multiple(colour_row: list, border_size: int) -> list:
    ...
    with the appropriate arguments. (For extra practice, try both ways!)
    """
    return crop_row(colour_row, border_size, len(colour_row) - 2 * border_size)
...
def blur_row(colour_row: list) -> list:
    ...
    """
    return[[round((colour_row[i][0] + colour_row[i - 1][0] + colour_row[i + 1][0]) / 3),
            round((colour_row[i][1] + colour_row[i - 1][1] + colour_row[i + 1][1]) / 3),
            round((colour_row[i][2] + colour_row[i - 1][2] + colour_row[i + 1][2]) / 3)]
           for i in range(1, len(colour_row) - 1)]
