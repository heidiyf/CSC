...
class _Transaction:
    ...
    Representation Invariants:
        - self.recipient != ''
        - self.amount > 0
        - not (self.next is not None and self.next.prev_digest != 0 and self.next.nonce == 0) or /
...
class Ledger:
    ...
    Representation Invariants:
        - all({i != '' for i in self._balance})
        - all({self._balance[i] >=0 for i in self._balance})
        - (self.first is None and self._balance = {}) or (self.first is not None and  self._balance != {})
    ...
    def record_transfer(self, sender: str, recipient: str, amount: int) -> None:
        ...
        """
        new_transfer = _Transaction(sender=sender, recipient=recipient, amount=amount)

        if self._balance[sender] < amount or sender not in self._balance:
            raise ValueError

        self.last.next = new_transfer
        self.last = self.last.next
        self._balance[sender] = self._balance[sender] - amount
        if recipient in self._balance:
    ...
    def record_mining(self, miner: str, amount: int) -> None:
        ...
        - Remember to update self.last and self._balance (possibly in addition to self.first)
        """
        new_mining = _Transaction(sender='', recipient=miner, amount=amount)

        if self.first is None:
            self.first = new_mining
            self.last = new_mining
            self._balance[miner] = amount
        else:
            self.last.next = new_mining
            self.last = self.last.next
            if miner in self._balance:
    ...
    def verify_balance(self) -> bool:
        ...
        while curr is not None:
            if curr.sender == '':
                balance_so_far += curr.amount
            curr = curr.next
        return sum({self.get_balance(i) for i in self._balance}) == balance_so_far
    ...
    def record_transfer_digest(self, sender: str, recipient: str, amount: int) -> None:
        ...
        self.last.next = new_transfer
        prev_digest = digest_from_hash(self.last)
        self.last = self.last.next
        self.last.prev_digest = prev_digest
        self._balance[sender] = self._balance[sender] - amount
        if recipient in self._balance:
            self._balance[recipient] = self._balance[recipient] + amount
        else:
    ...
    def verify_digests(self) -> bool:
        ...
        while curr.next is not None:
            if (curr.next.prev_digest != digest_from_hash(curr)) or (curr.next.prev_digest == 0):
                return False
            curr = curr.next
        return True
    ...
    def record_mining_digest_and_nonce(self, miner: str, nonce: int) -> None:
        ...
        """
        new_mining = _Transaction(sender='', recipient=miner, amount=MINING_AMOUNT, prev_digest=0, nonce=nonce)

        if self.first is None:
            digest = digest_from_hash_with_nonce(new_mining)
            ...
            prev_digest = digest_from_hash(self.last)
            new_mining = _Transaction(sender='', recipient=miner, amount=MINING_AMOUNT, prev_digest=prev_digest,
                                      nonce=nonce)
            digest = digest_from_hash_with_nonce(new_mining)
            if digest % 10 ** DIFFICULTY_RATING == 0:
                self.last.next = new_mining
...
def generate_nonce(ledger: Ledger, miner: str) -> int:
    ...
    if ledger.first is None:
        for x in range(10000):
            new_mining = _Transaction(sender='', recipient=miner, amount=MINING_AMOUNT, prev_digest=0, nonce=x)
            if digest_from_hash_with_nonce(new_mining) % 10 ** DIFFICULTY_RATING == 0:
                return x

    else:
        prev_digest = digest_from_hash(ledger.last)
        for x in range(10000):
            new_mining = _Transaction(sender='', recipient=miner, amount=MINING_AMOUNT, prev_digest=prev_digest,
                                      nonce=x)
            if digest_from_hash_with_nonce(new_mining) % 10 ** DIFFICULTY_RATING == 0:
